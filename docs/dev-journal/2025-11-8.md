...
export function MenuCard({ menu }: MenuCardProps) {
  ...
  return (
    <Card className="mb-3">
      ...
      {menu.memo && (
        <CardContent>
          <Text className="text-muted-foreground text-sm">{menu.memo}</Text>
        </CardContent>
      )}
    </Card>
  );
...
export function EmptyMenu({ dayLabel, onAddPress }: EmptyMenuProps) {
  return (
    <View className="border-border bg-muted/20 items-center justify-center rounded-lg border-2 border-dashed px-4 py-8">
      <Text className="text-muted-foreground mb-4 text-center">{dayLabel}の献立がありません</Text>
      {onAddPress && (
    ...
    <SafeAreaView className="bg-background flex-1" edges={['bottom']}>
      ...
      <ScrollView className="bg-background flex-1">
        ...
        <View className="border-border bg-card border-b p-4">
          ...
          <Text className="text-muted-foreground text-sm">月曜日から日曜日までの7日分</Text>
        </View>
...
ユーザー情報ヘッダーを実装してみたが不要と判断

<details>

<summary>判断理由 AI調べ</summary>
...
2. 毎回見る必要がない
   - 誰がログインしているかは一度確認すれば十分
   - アバターや名前は固定情報

3. ログアウトの頻度
   - スマホアプリでログアウトすることは少ない
   - むしろ設定画面にあるべき
...
[![Image from Gyazo](https://i.gyazo.com/993c882fb90167d51eb69270600fbeaf.png)](https://gyazo.com/993c882fb90167d51eb69270600fbeaf)
...
マイグレーションの時、`drizzle.config.ts`
`config({ path: '.env.local' });` こうしないと `.env.local` から url を取得できなかった

<details>

<summary>マイグレーションのコマンド</summary>
